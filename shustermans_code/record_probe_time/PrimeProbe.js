const NUM_OF_SAMPLES = 20; //2500 for 30s at 12ms per sample
...
let resultArray = new Array(Math.ceil(MEASUREMENT_TIME_IN_MS / SAMPLING_PERIOD_IN_MS));
...
function preformInterpolatedMeasurement() {
    let timeArr = new Array(NUM_OF_SAMPLES);
    let countTillEdgeArr = new Array(NUM_OF_SAMPLES);
...
function recordProbeAllSetsTime(){
    console.log("Recording probe all sets time...");
    let timeArr = new Array(NUM_OF_SAMPLES);
    for(let i=0; i<NUM_OF_SAMPLES; i++){
        let start = performance.now();
        PP.probeAllSets();
    ...
    // Download
    console.log("Downloading probe all sets time...");
    const blob = new Blob([csv], { type: "text/csv" });
    const link = document.createElement("a");
    link.href = URL.createObjectURL(blob);
    link.download = `probe_all_sets_time.csv`;
    link.click();
}


setTimeout(() => {
    recordProbeAllSetsTime();
